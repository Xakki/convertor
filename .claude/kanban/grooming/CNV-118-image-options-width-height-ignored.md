...
- Cross-worker tests используют одинаковые fixtures и проверяют детерминированные инварианты: при одной заданной стороне вычисленная сторона равна ближайшему целому от aspect-ratio calculation и не меньше 1 px; после округления ни одна сторона не превышает соответствующую заданную bounding-box maximum; при заданных обеих сторонах сохраняется aspect ratio и не возникает upscale сверх maxima; image-worker и browser worker (после своих prerequisites) дают одинаковые dimensions для одинакового normalized input.
...
- 2026-09-01: карточка остаётся в `grooming`, поскольку сохраняются открытые вопросы о supported inventory, фактических browser/animated pairs и текущем no-upscale поведении; перенос в `todo/` преждевременен. Решение по округлению записано и больше не является open question.

**Execution Log:**
- 2026-09-01: записано одобренное nearest-integer rounding с minimum 1 px и post-rounding bounding-box limit; вопрос об округлении удалён из Open questions. Inventory supported pairs, browser/animated boundaries и no-upscale policy не выбирались молча; карточка сохранена в `grooming`.
