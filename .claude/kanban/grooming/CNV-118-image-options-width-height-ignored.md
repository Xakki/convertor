...
- Какой точный browser/animated pair inventory и кто его утверждает? Scope текущей image migration policy уже включает все catalog rows, у которых `category`/`execution` равен `image` и есть image profile assignment, включая document-target image routes; `SVG→ICO` и OCR сохраняют специальные policies.
