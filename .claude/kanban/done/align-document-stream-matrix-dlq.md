...
**Decisions:**
- Matrix source-of-truth: long-term = **dynamic worker self-registration in DB** (Q2.1) → moved to NEW epic `registry-00-self-registration` (not this card).
...
- ⚠ UPDATE [USER DECISION 2026-07-01]: long-term those Stage-7 pairs **will disappear** — once `registry-00-self-registration` Phase 2 lands, the matrix holds only live-worker-declared pairs and the API 400-rejects unhandled pairs at submit (see `registry-00-self-registration`). This **supersedes** the "do NOT remove" line above going forward. Until then, THIS card's `PermanentError` fast-DLQ is the interim coverage.
