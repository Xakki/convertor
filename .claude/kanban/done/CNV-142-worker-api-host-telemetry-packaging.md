...
- 2026-09-06: историческая граница приёмки — только CNV-142 была переведена из
  `progress/` в `ready/`; это состояние superseded последующим `ready → done`
  завершением. Родительские и соседние карточки тогда не изменялись. Merge,
  push, release и повторный deploy не выполнялись.
- 2026-09-08: Пользователь явно разрешил завершить все текущие карточки из `ready/`; это историческое разрешение оформило CNV-142 как `done` единственным переходом `ready → done`.
