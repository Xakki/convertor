...
**Status:** ready — ревью CLEAN, QA зелёный; требует ручной проверки UI /admin#workers в браузере перед приёмкой эпика
...
- Вердикт ревью: CLEAN, блокеров нет. Подтверждено реальными DB-тестами (не моками): доступ только `ROLE_ADMIN` (атрибут + правило firewall, стоящее ВЫШЕ catch-all в security.yaml — не проваливается в более мягкое правило); TTL флага «жив/устарел» из единого env, второго литерала нет; seed-строки структурно не могут спрятать реального устаревшего воркера, т.к. `__seed__` запрещён реальным воркерам ещё на регистрации; данные из того же `findAllCapabilities()`, что и роутинг; `status` и `stale` — два независимых сигнала; `/admin/queues` не тронут, `getCapabilityWarnings()` не продублирован.
- Закрыт nit: формула порога устаревания вынесена в общий `WorkerLivenessTtl::staleThreshold()`, через который ходят и GC, и админ-провайдер — определение границы больше не может разойтись.
- ОСТАЁТСЯ РУЧНАЯ ПРОВЕРКА: реактивность Alpine, раскрытие матрицы по клику, 15-секундный поллинг и вёрстка Tailwind кодом не проверяемы — нужен живой браузер на `/admin#workers` перед закрытием эпика. Компиляция шаблона через полную цепочку include подтверждена HTTP-тестом.
- Коммиты: `8ee6ec7` (endpoint + провайдер + Twig + SPA), `ae18aad` (вынос общего порога TTL).
