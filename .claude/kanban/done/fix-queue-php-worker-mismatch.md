...
**Robustness fixes — DONE + verified (commit `4f18b3e`):**
- **result-consumer EM-resilience (queue-impl):** persister now obtains the EM lazily via `ManagerRegistry::getManager()` per call (was constructor-injected → stayed stale after close); command wraps each message in try/catch → on error `resetManager()` if closed + `XADD conv.result.dead` (DLQ) + `XACK` (no infinite redelivery); missing-conversion still ack-skips. Persister decoupled from `final S3Storage` (now takes `$resultsBucket` string, bound in services.yaml). `RedisConnectionFactory` de-`final`ed for test doubling. **5 PHPUnit tests pass, PHPStan green.**
- **worker BLOCK-timeout survival (py-worker):** dedicated blocking redis conn `socket_timeout=None` + `XREADGROUP` wrapped to swallow `TimeoutError`/`ConnectionError` and continue (was crashing the worker → restart loop). **+2 tests (stream_consumer 13 passed isolated).**
