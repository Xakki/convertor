...
- Вердикт ревью: CLEAN, блокеров нет. Подтверждено независимо: liveness НИКОГДА не создаёт строку (known/unknown из SELECT, а не из счётчика изменённых строк — идемпотентный повтор с тем же `lastSeenAt` не рапортует живой воркер как unknown); защита seed-строк от GC доказана per-row (stale seed выживает рядом со stale non-seed в одном проходе); `status` не участвует в роутинге ни в одну сторону (в коде и тестами); seed-строки получают `unknown` через DEFAULT колонки без зависимости от мутабельного состояния; инвалидация кэша GC срабатывает ровно при реальном удалении.
- Gateway: пуш не может застопорить WS-loop (отдельная asyncio-задача, все сбои сводятся к нераскидывающему результату), очередь маркеров отключения ограничена 2000, `instanceId` в `ready`-кадре — тот же `_instance_id()`, что и в `register()`, join по составному ключу не разъедется; старый воркер без поля не трекается, но раздача задач не затронута.
- Закрыты две мелочи: тест идемпотентного повтора (тот же `lastSeenAt` дважды → по-прежнему `updated`, не `unknown`, дубля строки нет) — фиксирует SELECT-дизайн от будущего регресса; и формулировки докблоков (PHP и gateway) исправлены: unknown-состояние держится не «один цикл», а до ближайшего реконнекта воркера.
- Известный форвард-луккинг зазор (строку воркера собрал GC, пока WS открыт → её уникальные пары исчезают из `/formats` до реконнекта) вынесен grooming-карточкой `[[liveness-orphaned-capability-reregister]]` — не блокер, сегодня радиус мал (все 6 типов имеют seed-строку).
- Коммиты: `0839da6` (PHP endpoint + GC + status), `8c53cc1` (gateway push), `a3ff27f` (тест идемпотентности + докблок), `8ab1dde` (формулировка в gateway).
