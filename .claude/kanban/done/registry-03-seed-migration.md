...
- Ревью seed-миграции: вердикт CLEAN. Исключение Stage-7 пар проверено поэлементно против всех 6 реальных Python-матриц — множества совпали в обе стороны (ничего лишнего не попало, ничего нужного не потеряно). Отсутствие строки `markup` в seed подтверждено как верное.
- Отдельная проверка переключения (диф хардкод ↔ seeded-БД): матрица из БД активируется сразу после seed, поэтому хардкод-fallback недостижим уже с этой карточки, а не с `[[registry-05-drop-hardcode]]`. Пропавших рабочих пар нет — все 57 исчезнувших никогда не работали в реальных воркерах.
- Найдена и исправлена тихая регрессия маршрутизации: `pdf→docx/md/txt` уходили в image/OCR-воркер вместо document. Решение пользователя — тай-брейк в реестре, воркеры остаются flag-agnostic. Порядок приоритета покрывает все 7 case-ов `FileCategory`, ничья решается детерминированно и не зависит от порядка строк в БД. Флаговый OCR-путь не задет (`isOcrSupported()` не читает матрицу).
- Исправлено: `instanceId='__seed__'` зарезервирован и отвергается на регистрации — иначе реальный воркер с таким значением был бы удалён откатом миграции.
- Побочные находки вынесены карточками: `[[migrate-diff-schema-drift]]`, `[[phpstan-skips-migrations]]`, `[[formats-catalogue-shrink]]`.
- Коммиты: `d5f5377` (seed), `c498bd0` (резерв `__seed__`), `9ee2798` (тай-брейк), `a9901bb` (ранги всех категорий).
