...
**Влияние:**
Ширина продукта (матрица форматов в `ROADMAP.md`) не подтверждена; часть заявленных конвертаций может молча не работать. Воркеры на Redis-LISTS не получают задачи в новой очередной модели (PHP пишет в Streams).

**Решение:**
Раздробить эпик per-worker. Каждая subcard валидирует свой воркер против матрицы форматов `ROADMAP.md` (справочные данные) и (где нужно) мигрирует его с Redis-LISTS на KeyDB Streams и на S3 in/out. Зонтичная карточка трекает прогресс и держит сквозные решения.
...
- Матрица форматов `ROADMAP.md` (за вычетом MVP-deferred) провалидирована end-to-end.
