...
**Status:** ready — ревью CLEAN, QA зелёный (см. лог ниже); ждёт финальной приёмки в составе эпика [[registry-00-self-registration]]
...
#### Ревью и финальный QA (2026-07-22)

- Ревью: вердикт CLEAN, блокеров нет. Отдельно подтверждены wire-совместимость (`instanceId` — совпадают регистр и charset-регулярка с обеих сторон) и заявление о том, что union матриц по нескольким строкам одного `workerType` уже работал корректно (изменений кода не потребовалось, добавлены docblock и регресс-тест).
- Приняты и исправлены 3 замечания: удалён осиротевший `WorkerCapability::update()`; в `down()` миграции добавлена проверка дублей по `worker_type` с явным исключением вместо падения на duplicate-key; при ОТСУТСТВИИ ключа `isAi` в CAPABILITIES теперь пишется warning (явный `isAi: False` остаётся тихим, эффективное значение не изменилось).
- Отклонены как несущественные: теоретическая коллизия `instanceId` по совпадающему 128-символьному префиксу; количество запросов в `upsert()` (они read-after-write, `refresh()` несущий).
- Финальный QA на ветке: `make phpstan` — 0 ошибок; `make cs-check` — чисто; `make test-php-live` — 425 тестов / 1733 assertions зелёные (1 известный pre-existing Notice, не регрессия); `make test-python` — 307 passed, 1 xfailed, 2 skipped.
- Побочная находка, вынесена отдельной grooming-карточкой `[[migrate-diff-schema-drift]]`: `make migrate-diff` выдаёт посторонний дрейф по другим таблицам, миграция этой карточки написана вручную.
- Коммиты: `3e3ccd2` (Python), `c2802d2` (PHP), `f1b3e6b` + `3e11c35` (ревью-фиксы).
