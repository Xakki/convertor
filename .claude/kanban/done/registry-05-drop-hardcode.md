...
- Вердикт ревью: CLEAN. Подтверждено независимо: непустых фолбэков не осталось ни на одном вырожденном пути; `&$save` — настоящий контракт Symfony `CacheInterface`; `repository === null` недостижим в проде (ноль вхождений `new ConversionRegistry(` в `src/` и `bin/`, автовайринг всегда подставляет реальный сервис); поведение OCR не изменилось (тела `isOcrSupported()`/`streamFor(ocr: true)` не тронуты); `md->html = document` сверено с golden-фикстурой — тест действительно был неверен, а не «подогнан».
- TOCTOU из `[[registry-04-matrix-tooling-tests]]` закрыт фактически: внутренний второй запрос к БД теперь тоже сходится к пустой матрице, которую ловит существующая проверка в `bin/dump-matrix.php`. Правок в самом скрипте не потребовалось.
- Находка, которой не было ни в карточке, ни в брифе: `buildMatrix()` кэширует результат на час, поэтому наивный возврат пустой матрицы при сбое БД ЗАКЭШИРОВАЛСЯ бы — секундный сбой превратился бы в часовой простой `/formats`. Тот же класс отказа, что и хардкод-фолбэк, только вывернутый: молчаливая пустая заморозка вместо молчаливой непустой подстановки. Решено через `bool &$save`: пустой/ошибочный результат возвращается, но не сохраняется.
- Закрыты два пробела покрытия: (1) тест сверяет `ConversionRegistrySeedFixture` с `seedRows()` реальной миграции через рефлексию — иначе фикстура могла бы молча разойтись с продом, а восемь тестовых файлов вечно зеленели бы против несуществующей матрицы; (2) тест на счастливый путь кэша — здоровый результат обязан кэшироваться, иначе каждый запрос идёт в БД без единого падающего теста. Оба проверены на срабатывание: временная поломка гарантии реально роняет тест.
- Golden-дельта: НОЛЬ — подтверждает, что удалён недостижимый код, а не изменено поведение.
- Коммиты: `7ce0b2c` (удаление хардкода, 161 строка), `18ad6e3` (два теста-сторожа).
