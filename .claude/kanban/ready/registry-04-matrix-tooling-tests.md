...
**Status:** ready — ревью CLEAN, QA зелёный; ждёт финальной приёмки в составе эпика [[registry-00-self-registration]]
...
#### Итог ревью (2026-07-22)

- Вердикт ревью: CLEAN, блокеров нет. Подтверждено независимо: скипов в drift-тесте не осталось, все пути отказа ведут в `pytest.fail`; `bin/dump-matrix.php` не может молча отдать хардкод (проверки идут напрямую по репозиторию, до создания реестра); golden-фикстура сходится ровно по 4 классам изменений (57 удалено + 36 добавлено + 32 смены категории audio→video + 10 смен `markup`→`document`), пятого необъяснённого класса нет; фильтр по `__seed__` даёт реальную изоляцию, т.к. `dama/doctrine-test-bundle` в проекте отсутствует.
- Цена односторонних проверок (вместо равенства) названа явно: незамеченной проходит пара, оставшаяся в capability-строке, но удалённая из Python-матрицы воркера, если routing-key покрыт другими его парами. Это случай, отданный по дизайну механизму TTL (`[[registry-06-liveness-push]]`), а не drift-тесту.
- Замечание про молчаливый `continue` в `_load_workers()` исправлено, и при проверке вскрылся более серьёзный случай: базовый класс `StreamConsumerBase` объявляет `CAPABILITIES: dict = {}`, поэтому у наследника с переименованным атрибутом `hasattr` остаётся истинным и проверка на `None` его НЕ ловит. Помогла только явная проверка обязательных ключей. Заодно закрыты ещё два входа того же вида (пустой список файлов воркеров, exit-0 с пустой матрицей).
- Находка TOCTOU в `bin/dump-matrix.php` (второй независимый запрос к БД внутри `getSupportedFormats()`) сознательно не чинилась здесь — она закрывается в `[[registry-05-drop-hardcode]]`, где фолбэку станет некуда проваливаться; зафиксирована там критериями приёмки.
- Побочные находки вынесены карточками: `[[no-ci-pipeline]]` (в репозитории нет CI вообще — поэтому мёртвый гейт и не был замечен ~12 дней), `[[phpstan-skips-migrations]]` (расширена: `bin/` тоже вне PHPStan и php-cs-fixer).
- Коммиты: `2418d1b` (PHP: dump-matrix + golden на seeded DB), `60a5e16` (Python: drift на register-round-trip, без скипов), `9cf17a3` (`test-drift` вписан в `test-php-live`), `133bd0a` (ужесточение входных проверок drift-теста; примечание: коммит смешанный, туда же попали правки карточек registry-05/06 — историю НЕ переписываем).
