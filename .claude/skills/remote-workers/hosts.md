...
| CPU/GPU | CPU only. |
| `EXT_FLUENT_PORT` | **Should** be `127.0.0.1:24224` (loopback-only — docker logging-driver on host → sidecar). **Observed live 2026-08-24: `0.0.0.0:24224`** (`.env.local` content + `ss -tlnp` both confirm the sidecar is listening on all interfaces, UDP and TCP). This is a known-but-uncarded gap — CNV-17 (done, 2026-08-01) flagged "uBook `.env.local` may have `0.0.0.0` — ops to rebind when convenient" and it was never turned into a follow-up card. Filed now as `.claude/kanban/grooming/CNV-122-ubook-fluent-port-exposed.md`. **Do not rebind it as a side effect of an unrelated task** — it's ops work on a live host, out of scope here; just don't make it worse and don't assume it's fixed. |
