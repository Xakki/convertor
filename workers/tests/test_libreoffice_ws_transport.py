"""WS-transport tests for LibreOfficeWorker (s1-11)."""
...
async def test_libreoffice_ws_ready_frame(tmp_path):
    """LibreOfficeWorker → WS ready{workerType:'document'}."""
    ...
    async with serve(gw_handler, "127.0.0.1", 0) as server:
        ...
        cfg = make_ws_cfg(port, tmp_path, "document", "test-libreoffice")
        client = WsClient(
            cfg, LibreOfficeWorker().process_job,
        ...
        try:
            ...
            assert rf["workerType"] == "document"
            assert rf["workerId"] == "test-libreoffice"
        finally:
            client.stop()
            await asyncio.wait_for(runner, timeout=3.0)


async def test_libreoffice_ws_inline_result(tmp_path):
    """LibreOfficeWorker: job{docx→pdf} → inline result frame via WS (convert mocked)."""
    from websockets.asyncio.server import serve
    from workers.libreoffice.worker import LibreOfficeWorker
    ...
    async with serve(gw_handler, "127.0.0.1", 0) as server:
        ...
        client = WsClient(
            ...
            http_client=httpx.AsyncClient(transport=mock_http_transport(b"fake-docx")),
        )
        runner = asyncio.create_task(client.run())
        try:
            await wait_for(lambda: any(f.get("type") == "ready" for f in received))
            ...
            with patch.object(worker, "convert", return_value=(str(out), "application/pdf", "pdf")):
