...
class Config:
    ...
    # Документ — 5 мин: конвертация через LibreOffice может быть медленной; image — 2 мин:
    # короткие операции, но порог остаётся запасом от двойной обработки. Audio — 5 мин и
    # video — 10 мин: ffmpeg-работы могут быть длительными. Data — 3 мин: обычно легче,
    # но внешние/табличные форматы не мгновенны. AI — 5 мин: удалённые inference-вызовы
    # вариативны. CNV-81-04 добавляет fast retry ТОЛЬКО до начала обработки, поэтому
    # доказательств снижать эти safety-пороги нет: сохраняем все значения.
